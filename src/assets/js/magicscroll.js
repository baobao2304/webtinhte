...
window.MagicScroll = (function() {
    ...
    u = v = (function() {
        ...
        (function() {
            ...
            if (!L.browser.mobile && L.browser.platform === "mac" && L.browser.touchScreen) {
                L.browser.mobile = true;
                L.browser.platform = "ios"
            }
            ...
            try {
                ...
                document.documentElement.setAttribute("data-magic-engine", L.browser.engine);
                document.documentElement.setAttribute("data-magic-engine-ver", L.browser.version)
            } catch (X) {}
        ...
        (function(W) {
            ...
            W.Event.Custom.mousedrag.handler = {
                add: function() {
                    ...
                    var X = W.Event.Custom.mousedrag.handler.handleMouseUp.jBindAsEvent(this);
        ...
        (function(Y) {
            ...
            Y.Event.Custom.pinch.handler = {
                ...
                add: function(al) {
                    ...
                    this.jAddEvent(["touchend", "pointerup"], Y.Event.Custom.pinch.handler.onTouchEnd, 1);
    ...
    var o;
    var p = function() {
        return "mgctlbxN$MSC mgctlbxV$" + "v2.0.42".replace("v", "") + " mgctlbxL$" + "t".toUpperCase() + ((window.mgctlbx$Pltm && "string" == v.jTypeOf(window.mgctlbx$Pltm)) ? " mgctlbxP$" + window.mgctlbx$Pltm.toLowerCase() : "")
    };
    ...
    b.prototype = {
        ...
        _initDragOnScroll: function() {
            var Q = (this.allSize / this.l) * this.itemStep;
            var af = 0;
            var T = false;
            var ai, L, ag, X, ah, K, G = (this.p_.pos == "left") ? "x" : "y",
                M = {
                    x: 0,
                    y: 0
                },
                V = this.o.effect == "scroll",
                Y, ab = true,
                P = {
                    x: 0,
                    y: 0
                ...
                H = j(function(al) {
                    var ak, aj = 0;
                    if (al > this.containerWidth) {
                        al = this.containerWidth
                    }
                    for (ak = 1.5; ak <= 90; ak += 1.5) {
                        aj += (al * Math.cos(ak / Math.PI / 2))
                    }
                    return this.containerWidth > aj ? aj : this.containerWidth
                }).jBind(this),
                J = j(function(al) {
                    var am, aj = 0,
                        ak, an;
                    while (aj > this.containerPosition) {
                        aj -= this.containerWidth
                    }
                    if (Math.abs(aj - this.containerPosition) > this.containerWidth / 2) {
                        aj += this.containerWidth
                    }
                    an = aj;
                    for (am = 0; am < this.globalLength; am++) {
                        ak = parseInt(this.container.childNodes[am].getAttribute("data-item"));
                        if (an == 0) {
                            this.last = ak;
                            break
                        }
                        an += this.items[ak].size[this.p_.size]
                    }
                    return aj
                }).jBind(this),
                ad = j(function(aj) {
                        ...
                        if (!ab) {
                            ...
                            if (this.o.effect == "scroll") {
                                if (L > 200) {
                                    ...
                                    K = H(Math.abs(P[G] - an[G]))
                                }
                                if (v.browser.mobile && this.o.step !== "auto") {
                                    var am = j(function() {
                                        var aq = this.itemStep;
                                        var ap = this.allSize / this.l;
                                        for (var ao = 0; ao < this.itemStep - 1; ao++) {
                                            if (ag === "forward") {
                                                if (af > ap * (ao + 1)) {
                                                    aq--
                                                } else {
                                    ...
                                    var ak = am();
                                    var aj = (this.allSize / this.l) * (ak - 1);
                                    K = aj
                                }
                                ah = K;
                                if ("infinite" === this.o.loop) {
                                    this.distance = Math.abs(ah);
                                    this._shiftContainer(ag)
                                }
                                if ("infinite" === this.o.loop || this.containerPosition <= 0) {
                                    if (Math.abs(this.containerPosition) < ah) {
                                        ah = Math.abs(this.containerPosition)
                                    }
                                    this.containerPosition -= ah
                                }
                                ag == "forward" ? this.containerPosition -= this.correctItemPosition(ag) : this.containerPosition += this.correctItemPosition(ag);
                                if (!this.o.loop || "rewind" === this.o.loop) {
                                    this.jCallEvent("enable");
                                    this.loop.firstItem = false;
                                    this.loop.lastItem = false;
                                ...
                                "infinite" === this.o.loop && this._shiftContainer(ag);
                                if (L < 200) {
                                    this.distance = this.containerWidth;
                                    "infinite" === this.o.loop && this._shiftContainer(ag);
                                    if (ag == "forward") {
                            ...
                            M[G] = this.containerPosition;
                            this.container.jAddEvent("transitionend", j(function(ao) {
                                if (ao.target == this.container) {
                                    F()
                                }
                            }).jBind(this));
                            if (al == this.containerPosition) {
                                this.move_ = false;
                                V = false;
                                ab = true
                            }
                            this.container.jSetCssProp("transition", g + " " + Y + "ms cubic-bezier(.22,.63,.49,.8)");
                            this.container.jSetCssProp(g, "translate3d(" + M.x + "px, " + M.y + "px, 0)")
                        } else {
                            if (!v.browser.mobile) {
                                F()
                ...
                ae = j(function() {
                    var aj = O * 0.2;
                    if (Math.abs(aj) < 0.0001) {
                        R();
                        return
                    }
                    O -= aj;
                    this.containerPosition -= aj;
                    M[G] = this.containerPosition;
                    this.container.jSetCssProp(g, "translate3d(" + M.x + "px, " + M.y + "px, 0)");
                    aa = setTimeout(ae, 16)
                }).jBind(this),
                ac = j(function(ak) {
                    if (Z) {
                        var aj = ak[G] - S > 0 ? "backward" : "forward";
                        ab = false;
                        if ("infinite" === this.o.loop) {
                            this.distance = Math.abs(ah);
                            this._shiftContainer(aj)
                        }
                        if (v.browser.ieMode) {
                            O += ah;
                            if (!U) {
                        ...
                        this.freeTouchPreload(aj)
                    }
                }).jBind(this);
            this.onDrag = j(function(aj) {
                if (this.stopScroll || this.o.effect == "animation" && V) {
                    return
                }
                if ("dragstart" == aj.state) {
                    ai = aj.timeStamp;
                    P.x = aj.x;
                    P.y = aj.y;
                    S = aj[G];
                    ...
                    ag = (ah > 0) ? "forward" : "backward";
                    ah = S - aj[G];
                    this.moveSettings.direction = ag;
                    if ("dragend" == aj.state) {
                        if (I) {
                            I = false;
                            T = false;
                        ...
                        if (this.o.orientation == "vertical" || Math.abs(aj.x - P.x) > Math.abs(aj.y - P.y)) {
                            aj.stopDefaults();
                            if (!I) {
